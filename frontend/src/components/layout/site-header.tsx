    ...
    <header className="sticky top-0 z-50 flex w-full shrink-0 items-center border-b bg-background">
      <div className="flex h-14 w-full items-center gap-4 px-8 font-sans">
        <Link to="/" className="shrink-0" aria-label="Filecano home">
          <Icon className="size-8" markClassName="size-8" />
        </Link>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  ...
                  <div className="hidden min-w-0 flex-1 text-start text-sm leading-tight sm:grid">
                    <span className="truncate font-medium">{user.name}</span>
                    <span className="truncate text-xs text-muted-foreground">
                      {user.email}
                    </span>
                  </div>
                  <ChevronsUpDown className="hidden size-4 shrink-0 sm:block" />
                </Button>
              </DropdownMenuTrigger>
